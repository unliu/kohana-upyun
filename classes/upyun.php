<?php
class Upyun{//liuxun Upper case to Lower case 2012/7/1
	...
	* return UpYun object
	*/
	public function __construct() {//liuxun 2012/7/1 $bucketname, $username, $password

		//liuxun add 2012/7/1 3 lines:Load config from config file
		$bucketname = Kohana::$config->load('upyun.bucketname');
		$username = Kohana::$config->load('upyun.username');
		$password = Kohana::$config->load('upyun.password');
	...
	* return Upyun object;
	*/
	public static function factory($type = 'picture')
	{
		if ($type == 'picture')
		{
			return new Upyun();
		...
		{
			$upyun = new Upyun();
			$upyun->bucketname = $upyun->bucketname.'-other';//temp for testing -- and it works! liuxun 2012/7/1
			return $upyun;
		}
	}
